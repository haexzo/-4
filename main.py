...
def show_student_info():
    ...
    if student_name:
        ...
        stop_button.pack_forget()  # 종료 버튼 숨기기
    else:
...
def show_start_time():
    ...
    start_time_label.config(text=f"시작 시간: {start_time.strftime('%H:%M:%S')}")
    start_button.pack_forget()
...
def show_stop_time():
    ...
    stop_time_label.config(text=f"종료 시간: {stop_time.strftime('%H:%M:%S')}")
    study_duration = stop_time - start_time
    duration_seconds = study_duration.total_seconds()
    total_study_time_label.config(text=f"이번 공부 시간: {seconds_to_time(duration_seconds)}")

    # 바코드와 총 공부 시간을 텍스트 파일에 기록
    update_student_time(current_barcode, duration_seconds)
